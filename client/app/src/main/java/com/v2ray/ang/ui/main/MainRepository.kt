...
class MainRepository(
    ...
    override suspend fun replaceManagedServers(configs: List<String>): Int {
        if (configs.isEmpty()) return 0
        // importBatchConfig(..., append=false) publishes the replacement group atomically. Do
        // not clear storage first: the VPN daemon may still be using the selected profile after
        // a UI-process restart, and unrelated/temporary groups must remain untouched.
        val (count, _) = AngConfigManager.importBatchConfig(
            configs.joinToString("\n"),
            AppConfig.DEFAULT_SUBSCRIPTION_ID,
            false
        )
        val guids = MmkvManager.decodeServerList(AppConfig.DEFAULT_SUBSCRIPTION_ID)
        val selected = MmkvManager.getSelectServer()
            ?.takeIf { MmkvManager.decodeServerConfig(it) != null }
        if (selected == null) {
            guids.firstOrNull()?.let(MmkvManager::setSelectServer)
        }
        return count
    }

    override fun getManagedServerGuids(): List<String> =
        MmkvManager.decodeServerList(AppConfig.DEFAULT_SUBSCRIPTION_ID)

    override fun probeServerTcp(guid: String): Long {
        val profile = MmkvManager.decodeServerConfig(guid) ?: return -1L
        val host = profile.server?.takeIf { it.isNotBlank() } ?: return -1L
        val port = profile.serverPort?.toIntOrNull() ?: return -1L
        return SpeedtestManager.socketConnectTime(host, port, 1_200)
    }
