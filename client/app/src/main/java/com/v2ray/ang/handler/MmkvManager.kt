...
import com.v2ray.ang.haima.SorenKeyStore
...
object MmkvManager {
    ...
    private lateinit var cryptKey: String

    private fun encryptedStorage(id: String): MMKV =
        MMKV.mmkvWithID(id, MMKV.MULTI_PROCESS_MODE, cryptKey)

    private val mainStorage by lazy { encryptedStorage(ID_MAIN) }
    private val profileFullStorage by lazy { encryptedStorage(ID_PROFILE_FULL_CONFIG) }
    private val serverRawStorage by lazy { encryptedStorage(ID_SERVER_RAW) }
    private val serverAffStorage by lazy { encryptedStorage(ID_SERVER_AFF) }
    private val subStorage by lazy { encryptedStorage(ID_SUB) }
    private val assetStorage by lazy { encryptedStorage(ID_ASSET) }
    private val settingsStorage by lazy { encryptedStorage(ID_SETTING) }
    ...
    fun initialize(context: Context) {
        val keyStore = SorenKeyStore(context)
        val needsEncryptionMigration = !keyStore.hasMmkvCryptKey()
        val installCryptKey = keyStore.mmkvCryptKey()
        ...
        if (needsEncryptionMigration) {
            listOf(
                ...
                MMKV.mmkvWithID(id, MMKV.MULTI_PROCESS_MODE).reKey(installCryptKey)
            }
        }
        cryptKey = installCryptKey
    }
    ...
    internal fun saveServerProfiles(
        ...
        withProfileIndexLock {
            ...
            val selectedCandidate = ProfileReplacement.findSelectedReplacement(
                profiles = profiles,
                currentSelection = previousSelection,
                selectedProfile = selectedProfile,
            )

            // Keep the selected GUID stable while replacing a managed group. The daemon stores
            // this GUID for network-handover reloads and can outlive the UI process; publishing a
            // random replacement GUID would make a live VPN impossible to reload safely.
            val preserveSelectedGuid = !append &&
                previousSelection != null &&
                previousSelection in replacedServers &&
                selectedCandidate != null &&
                selectedCandidate != previousSelection &&
                previousSelection !in profiles
            val publishedProfiles = linkedMapOf<String, ProfileItem>()
            val publishedRawConfigs = mutableMapOf<String, String>()
            profiles.forEach { (guid, profile) ->
                val publishedGuid = if (preserveSelectedGuid && guid == selectedCandidate) {
                    requireNotNull(previousSelection)
                } else {
                    guid
                }
                publishedProfiles[publishedGuid] = profile
                rawConfigs[guid]?.let { raw -> publishedRawConfigs[publishedGuid] = raw }
            }
            val replacementSelection = if (preserveSelectedGuid) {
                previousSelection
            } else {
                selectedCandidate
            }

            publishedProfiles.forEach { (guid, profile) ->
                requireStorageWrite(
                    profileFullStorage.encode(guid, JsonUtil.toJson(profile)),
                    "Failed to save profile payload",
                )
                publishedRawConfigs[guid]?.let { raw ->
                    requireStorageWrite(
            ...
            publishedProfiles.keys.forEach { guid ->
            ...
            val removablePayloads = ProfileReplacement.findRemovablePayloads(
                ...
                replacementServers = publishedProfiles.keys,
