...
class CoreVpnService : VpnService(), ServiceControl {
    ...
    private var lastStartupFailure: String? = null
...
//    }
        ...
        return try {
            ...
            if (!setupVpnService()) {
                MessageHelper.sendMsg2UI(
                    ...
                    lastStartupFailure ?: "VPN setup failed"
                )
                unlockStart()
                stopSelf()
                return START_NOT_STICKY
            ...
            LogUtil.e(AppConfig.TAG, "StartCore-VPN: Unhandled startup failure", failure)
            MessageHelper.sendMsg2UI(
                this,
                AppConfig.MSG_STATE_START_FAILURE,
    ...
    private fun setupVpnService(): Boolean {
        ...
        if (prepare != null) {
            ...
            lastStartupFailure = "VPN permission is not granted"
            return false
        }

        if (configureVpnService() != true) {
            LogUtil.e(AppConfig.TAG, "StartCore-VPN: Configuration failed")
            if (lastStartupFailure.isNullOrBlank()) {
                lastStartupFailure = "VPN interface configuration failed"
            }
            return false
        }
        ...
        if (!runTun2socks()) {
            if (lastStartupFailure.isNullOrBlank()) {
                lastStartupFailure = "HEV VPN tunnel could not start"
            }
            return false
        }
        return true
    ...
    private fun configureVpnService(): Boolean {
        ...
        try {
            ...
            lastStartupFailure = e.message?.takeUnless { it.isBlank() }
    ...
    private fun runTun2socks(): Boolean {
        if (SettingsManager.isUsingHevTun()) {
            val hevService = TProxyService(
                context = applicationContext,
                vpnInterface = mInterface,
                isRunningProvider = { isRunning },
                restartCallback = { runTun2socks() }
            )
            tun2SocksService = hevService
            if (!hevService.startTun2Socks()) {
                lastStartupFailure = hevService.lastStartFailure
                return false
            }
        } else {
